  ...
  return (<>
    <style>{`
      ...
      *{margin:0;padding:0;box-sizing:border-box;}html,body{width:100%;height:100%;background:var(--black);font-family:'DM Sans',system-ui,sans-serif;color:var(--full);overflow:hidden;}
      ...
      .spotlight{background:linear-gradient(135deg,rgba(168,85,247,.14),rgba(168,85,247,.03));border:1px solid rgba(168,85,247,.22);border-radius:${14*V}px;padding:${20*V}px ${18*V}px;position:relative;overflow:hidden;animation:spotPulse 4s ease-in-out infinite;}
      @keyframes spotPulse{0%,100%{box-shadow:0 0 0 0 rgba(168,85,247,.15)}50%{box-shadow:0 0 ${30*V}px ${4*V}px rgba(168,85,247,.12)}}
      .spot-crown{position:absolute;right:${14*V}px;top:${12*V}px;font-size:${28*V}px;opacity:.4;}.spot-lbl{font-size:${9*V}px;letter-spacing:${3*V}px;text-transform:uppercase;color:#A855F7;margin-bottom:${8*V}px;}.spot-prod{font-size:${16*V}px;font-weight:700;margin-bottom:${4*V}px;padding-right:${36*V}px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}.spot-ch{font-size:${12*V}px;color:var(--dim);margin-bottom:${12*V}px;}.spot-amt{font-family:Bebas Neue,system-ui;font-size:${46*V}px;letter-spacing:${2*V}px;color:#C084FC;line-height:1;}.spot-amt small{font-size:${16*V}px;color:#A855F7;}
      .spot-stats{display:flex;gap:${8*V}px;margin-top:${14*V}px;}.spot-stat{flex:1;background:rgba(168,85,247,.08);border-radius:${8*V}px;padding:${8*V}px ${10*V}px;border:1px solid rgba(168,85,247,.12);}.spot-stat-val{font-family:Bebas Neue,system-ui;font-size:${18*V}px;color:#C084FC;line-height:1;}.spot-stat-lbl{font-size:${8*V}px;letter-spacing:${1.5*V}px;text-transform:uppercase;color:var(--dim);margin-top:${2*V}px;}
      ...
      <div className="sidebar">
        <div><div className="sb-title">Last Client · Largest Order</div>
          <div className="spotlight"><div className="spot-crown">👑</div><div className="spot-lbl">◆ Top Client</div>
            ...
            {bigClient&&<div className="spot-stats">
              <div className="spot-stat"><div className="spot-stat-val">1</div><div className="spot-stat-lbl">Items</div></div>
